...
bool GameLayer::init() {
    if (Layer::init()) {
        ...
        this->bird->setPosition(origin.x + visibleSize.width * 1 / 3 - 5,
...
//        groundBody->setContactTestBitmask(true);
        ...
        // init land
        this->landSpite1 = Sprite::createWithSpriteFrame(
        ...
        this->landSpite2 = Sprite::createWithSpriteFrame(
                AtlasLoader::getInstance()->getSpriteFrameByName("land"));
...
void GameLayer::scrollLand(float dt) {
    ...
    this->landSpite2->setPositionX(
            this->landSpite1->getPositionX() + this->landSpite1->getContentSize().width - 2.0f);
...
void GameLayer::createPips() {
    ...
    for (int i = 0; i < PIP_COUNT; i++) {
        Size visibleSize = Director::getInstance()->getVisibleSize();
        Sprite *pipUp = Sprite::createWithSpriteFrame(
                AtlasLoader::getInstance()->getSpriteFrameByName("pipe_up"));
        Sprite *pipDown = Sprite::createWithSpriteFrame(
        ...
        singlePip->setPosition(visibleSize.width + i * PIP_INTERVAL + WAIT_DISTANCE,
